...
data1=read.csv("F:/data hasil preprocess 1.csv",sep=",",dec='.')
...
imdb = read.csv("F:/data hasil preprocess 1.csv")
imdb$title=as.factor(imdb$title)
imdb$genre=as.factor(imdb$genre)
imdb$directors=as.factor(imdb$directors)
imdb$actor=as.factor(imdb$actor)
str(imdb)
...
server <- function(input, output) {
  ...
  output$clustdata <- renderTable(data.frame(data[,1],tsne_df))
